...
class Worker:
    ...
    def register_task(self, name: str, func: Callable, interval_seconds: int) -> None:
        """
            ...
            try:
                ...
                if available_slots > 0:
                    for name, task in self.tasks.items():
                        if (
                            name not in self.running_tasks
                            and task.should_run()
...
async def run_worker(concurrency: int = 10) -> int:
    ...
    worker.register_task(
        "blockchain_monitor", monitor_blockchain_activity, 60
    )  # Every minute
    ...
    worker.register_task(
        "security_check", check_security_alerts, 1800
    )  # Every 30 minutes
