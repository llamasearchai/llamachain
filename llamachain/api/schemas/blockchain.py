...
class EthereumBlockInfo(BlockBasic):
    ...
    base_fee_per_gas: Optional[int] = Field(
        None, description="Base fee per gas (EIP-1559)"
    )
...
class BlockResponse(BaseModel):
    ...
    is_cached: bool = Field(
        ..., description="Whether the block is cached in the database"
    )
    ...
    transactions: Optional[List[Dict[str, Any]]] = Field(
        None, description="Transactions in the block"
    )
...
class BalanceResponse(BaseModel):
    ...
    denomination: str = Field(
        ..., description="Denomination name (wei, lamports, etc.)"
    )
...
class GasPriceResponse(BaseModel):
    ...
    recommended_speeds: Optional[Dict[str, int]] = Field(
        None, description="Recommended gas prices for different speeds"
    )
    fixed_cost: Optional[bool] = Field(
        None, description="Whether the chain has fixed transaction costs"
    )
    prioritization_fees: Optional[bool] = Field(
        None, description="Whether the chain supports prioritization fees"
    )
...
class ChainInfoResponse(BaseModel):
    ...
    latest_block_timestamp: Union[int, float] = Field(
