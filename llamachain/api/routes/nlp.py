...
class NLPQueryRequest(BaseModel):
    ...
    generate_response: bool = Field(
        False, description="Whether to generate a natural language response"
    )
    translate_to_query: bool = Field(
        False, description="Whether to translate to a structured query"
    )
...
class NLPQueryResponse(BaseModel):
    ...
    structured_query: Optional[Dict[str, Any]] = Field(
        None, description="Structured query"
    )
...
async def process_query(request: NLPQueryRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        response = {
            ...
            "response": (
                processed_query["response"] if request.generate_response else None
            ),
            "structured_query": (
                processed_query["structured_query"]
                if request.translate_to_query
                else None
            ),
        }
...
async def analyze_query(
    ...
    generate_response: bool = Query(
        False, description="Whether to generate a natural language response"
    ),
    translate_to_query: bool = Query(
        False, description="Whether to translate to a structured query"
    ),
) -> Dict[str, Any]:
    """
...
async def classify_intent_endpoint(
    query: str = Query(..., description="Natural language query to classify")
) -> Dict[str, Any]:
    """
...
async def extract_entities_endpoint(
    query: str = Query(
        ..., description="Natural language query to extract entities from"
    )
) -> Dict[str, Any]:
    """
...
async def generate_response_endpoint(
    query: str = Query(
        ..., description="Natural language query to generate a response for"
    )
) -> Dict[str, Any]:
    """
