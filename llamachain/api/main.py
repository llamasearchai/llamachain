...
from llamachain.db.session import SessionLocal, engine
...
async def lifespan(app: FastAPI):
    ...
    app.state.eth_chain = EthereumChain(
        rpc_url=settings.ETH_RPC_URL, ws_url=settings.ETH_WSS_URL
    )
    await app.state.eth_chain.connect()

    app.state.sol_chain = SolanaChain(rpc_url=settings.SOL_RPC_URL)
...
async def blockchain_health():
    ...
    return {
        "ethereum": {
            ...
            "chain_id": (
                await app.state.eth_chain.get_chain_id() if eth_status else None
            ),
            "latest_block": (
                await app.state.eth_chain.get_latest_block_number()
                if eth_status
                else None
            ),
        },
