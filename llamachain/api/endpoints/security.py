    ...
    try:
        ...
        try:
            ...
            if use_ml:
                ...
                result = await auditor.analyze_contract_with_ml(
                    temp_file_path, ml_model
                )
            else:
            ...
            if contract_address:
                ...
                query = (
                    ...
                    .filter(
                        models.Contract.blockchain
                        == models.BlockchainEnum(chain.lower())
                    )
            ...
            raise HTTPException(
                status_code=500, detail=f"Error auditing contract: {str(e)}"
            )
        finally:
...
async def audit_contract_by_address(
    ...
    try:
        ...
        if use_ml:
            # Load ML model for vulnerability detection
            ml_model = VulnerabilityDetectionModel()
            result = await auditor.analyze_contract_address_with_ml(
                chain, address, ml_model
            )
        else:
            # Use only static analysis
...
async def get_contract_vulnerabilities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting vulnerabilities: {str(e)}"
        )
...
async def verify_zk_transaction(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error verifying transaction: {str(e)}"
        )
...
async def protect_from_mev(
    chain: str = Query(
        "ethereum", description="Blockchain (only ethereum supported currently)"
    ),
    tx_data: Dict[str, Any] = Body(..., description="Transaction data to protect"),
    protection_strategy: str = Query(
        "flashbots", description="Protection strategy to use"
    ),
):
    """
    ...
    if chain.lower() != "ethereum":
        raise HTTPException(
            status_code=400, detail="MEV protection currently only supports Ethereum"
        )
    ...
    try:
        ...
        result = await mev_protection.protect_transaction(
            tx_data, strategy=protection_strategy
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error protecting transaction: {str(e)}"
        )
