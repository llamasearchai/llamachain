...
async def get_chain_info(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting chain info: {str(e)}"
        )
...
async def get_block(
    ...
    try:
        ...
        if not block:
            raise HTTPException(
                status_code=404, detail=f"Block {block_number} not found"
            )
...
async def get_transaction(
    ...
    try:
        ...
        if not tx:
            raise HTTPException(
                status_code=404, detail=f"Transaction {tx_hash} not found"
            )
            ...
            response.update(
                {
                    ...
                    "signature": tx_hash,
                    "status": (
                        "success" if tx.get("meta", {}).get("err") is None else "failed"
                    ),
                    "fee": tx.get("meta", {}).get("fee", 0),
                    "recent_blockhash": tx.get("transaction", {})
                    .get("message", {})
                    .get("recentBlockhash", ""),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting transaction: {str(e)}"
        )
...
async def get_gas_price(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting gas price: {str(e)}"
        )


@router.get(
    "/address/{chain}/{address}/transactions", response_model=TransactionListResponse
)
async def get_address_transactions(
    ...
    request: Request = None,
    limit: int = Query(
        10, ge=1, le=100, description="Number of transactions to retrieve"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting transactions: {str(e)}"
        )
...
async def get_contract_code(
    ...
    try:
        ...
        if not code or code == "0x":
            raise HTTPException(
                status_code=404, detail=f"No contract found at address {address}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting contract code: {str(e)}"
        )
