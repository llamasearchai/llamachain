...
class EthereumChain(BlockchainBase):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if self.ws_url:
                try:
                    self.web3_ws = AsyncWeb3(
                        AsyncWeb3.AsyncWebsocketProvider(self.ws_url)
                    )
                    if await self.web3_ws.is_connected():
                        self.logger.info(
                            f"Connected to Ethereum node via WebSocket: {self.ws_url}"
                        )
                    else:
                        self.logger.warning(
                            f"Failed to connect to WebSocket: {self.ws_url}"
                        )
                        self.web3_ws = None
                except Exception as e:
            ...
            if is_connected:
                ...
                self.logger.info(
                    f"Connected to Ethereum node (chain_id: {chain_id}, latest_block: {latest_block})"
                )
                return True
            else:
                self.logger.error(
    ...
    async def call_contract(
        self, address: str, function_signature: str, args: List[Any]
    ) -> Any:
        """
        ...
        try:
            ...
            encoded_args = (
                ""  # This is simplified, actual encoding depends on argument types
            )
            ...
            self.logger.error(
                f"Error calling contract {address}.{function_signature}: {e}"
            )
            return None
    ...
    def _convert_block_data(self, block: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "transactions" in block:
            if block["transactions"] and isinstance(block["transactions"][0], dict):
                block["transactions"] = [
                    self._convert_transaction_data(tx) for tx in block["transactions"]
                ]
            else:
                # Only transaction hashes, convert to hex strings
                block["transactions"] = [
                    tx.hex() if hasattr(tx, "hex") else tx
        ...
        if "logs" in receipt and receipt["logs"]:
            ...
            for log in receipt["logs"]:
                ...
                if "topics" in log_dict and log_dict["topics"]:
                    log_dict["topics"] = [
                        t.hex() if hasattr(t, "hex") else t for t in log_dict["topics"]
                    ]
