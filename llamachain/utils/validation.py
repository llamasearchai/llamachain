...
def validate_address(
    address: str, blockchain: str = "ethereum"
...
def validate_tx_hash(
    tx_hash: str, blockchain: str = "ethereum"
) -> Tuple[bool, Optional[str]]:
    """
    ...
    if blockchain == "ethereum":
        ...
        if len(tx_hash_without_prefix) != 64:
            return (
                False,
                "Ethereum transaction hash must be 66 characters long (including '0x')",
            )

        if not re.match(r"^[0-9a-fA-F]{64}$", tx_hash_without_prefix):
            return (
                False,
                "Ethereum transaction hash must contain only hexadecimal characters",
            )
