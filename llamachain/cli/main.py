...
class BlockchainCommands(CLICommand):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        ...
        subparsers = parser.add_subparsers(
            dest="blockchain_command", help="Blockchain command"
        )
        ...
        address_parser = subparsers.add_parser(
            "address", help="Get address information"
        )
...
class SecurityCommands(CLICommand):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        ...
        vuln_parser = subparsers.add_parser(
            "vulnerabilities", help="List vulnerability types"
        )
...
class AnalyticsCommands(CLICommand):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        ...
        subparsers = parser.add_subparsers(
            dest="analytics_command", help="Analytics command"
        )
...
class NLPCommands(CLICommand):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        ...
        query_parser = subparsers.add_parser(
            "query", help="Process a natural language query"
        )
        query_parser.add_argument("--text", "-t", required=True, help="Query text")
        query_parser.add_argument(
            "--response", "-r", action="store_true", help="Generate response"
        )
        query_parser.add_argument(
            "--structured",
            "-s",
            action="store_true",
            help="Translate to structured query",
        )
        query_parser.add_argument(
            "--execute", "-e", action="store_true", help="Execute the query if possible"
        )
    ...
    async def _process_query(cls, args: argparse.Namespace) -> int:
        ...
        try:
            ...
            print(
                f"Intent: {processed_query['intent']['type']} ({processed_query['intent']['description']})"
            )
...
class ConfigCommands(CLICommand):
    ...
    async def run(cls, args: argparse.Namespace) -> int:
        ...
        if args.config_command == "list":
            ...
            print("Note: This feature is not yet implemented")
            print(
                "Please edit the .env file directly to change configuration settings."
            )
            return 0
