...
async def process_query(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if args.format == "json":
            ...
            print(
                f"Intent: {result['intent']['type']} ({result['intent']['description']})"
            )
    ...
    try:
        ...
        if args.format == "json":
            result = {
                ...
                "confidence": confidence,
                "confidence_scores": {
                    intent_type.value: score for intent_type, score in scores.items()
                },
            }
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="LlamaChain NLP Command-Line Interface"
    )
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process a natural language query", parents=[common_parser]
    )
    process_parser.add_argument("--query", "-q", required=True, help="Query to process")
    process_parser.add_argument(
        "--response", "-r", action="store_true", help="Generate a response"
    )
    process_parser.add_argument(
        "--structured", "-s", action="store_true", help="Generate a structured query"
    )

    # Intent command
    intent_parser = subparsers.add_parser(
        "intent", help="Classify intent of a query", parents=[common_parser]
    )
    intent_parser.add_argument("--query", "-q", required=True, help="Query to classify")

    # Entity command
    entity_parser = subparsers.add_parser(
        "entity", help="Extract entities from a query", parents=[common_parser]
    )
    entity_parser.add_argument(
        "--query", "-q", required=True, help="Query to extract entities from"
    )

    # Response command
    response_parser = subparsers.add_parser(
        "response", help="Generate a response for a query", parents=[common_parser]
    )
    response_parser.add_argument(
        "--query", "-q", required=True, help="Query to generate a response for"
    )
