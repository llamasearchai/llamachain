...
class ResponseTemplate:
    ...
    def fill(
        self, entities: List[Entity], additional_values: Optional[Dict[str, str]] = None
    ) -> str:
        """
    ...
    def __init__(self):
        ...
        self.templates = {
            Intent.GET_TRANSACTION: [
                ResponseTemplate(
                    "I'll retrieve information for transaction {tx_hash}.", ["tx_hash"]
                ),
                ResponseTemplate(
                    "Fetching details for transaction {tx_hash}.", ["tx_hash"]
                ),
                ResponseTemplate(
                    "Looking up transaction {tx_hash} on the {blockchain} blockchain.",
                    ["tx_hash", "blockchain"],
                ),
            ...
            Intent.GET_BLOCK: [
                ResponseTemplate(
                    "I'll fetch block {block_number} for you.", ["block_number"]
                ),
                ResponseTemplate(
                    "Retrieving block number {block_number} from the {blockchain} blockchain.",
                    ["block_number", "blockchain"],
                ),
                ResponseTemplate(
                    "Getting block {block_number} details.", ["block_number"]
                ),
                ResponseTemplate("I'll fetch the block information you requested."),
            ],
            Intent.GET_BALANCE: [
                ResponseTemplate(
                    "Checking the balance of address {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll retrieve the balance for {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
                ResponseTemplate("Looking up the balance of {address}.", ["address"]),
                ResponseTemplate(
                    "Checking the {token} balance for {address}.", ["address", "token"]
                ),
                ResponseTemplate("I'll check the account balance for you."),
            ],
            Intent.GET_CONTRACT: [
                ResponseTemplate(
                    "Retrieving information for contract {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll fetch details for the contract at {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
            ...
            Intent.GET_GAS_PRICE: [
                ResponseTemplate(
                    "Checking the current gas price on the {blockchain} network.",
                    ["blockchain"],
                ),
            ...
            Intent.GET_PRICE: [
                ...
                ResponseTemplate(
                    "I'll retrieve the latest price for {token}..", ["token"]
                ),
                ResponseTemplate(
                    "Getting the current market value of {token}.", ["token"]
                ),
                ResponseTemplate("I'll check the token price for you."),
            ],
            Intent.ANALYZE_CONTRACT: [
                ResponseTemplate(
                    "Analyzing the security of contract {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll audit the contract at {address} for security vulnerabilities.",
                    ["address"],
                ),
                ResponseTemplate(
                    "Starting security analysis for contract {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
                ResponseTemplate(
                    "I'll analyze the contract for security vulnerabilities."
                ),
            ],
            Intent.ANALYZE_WALLET: [
                ResponseTemplate(
                    "Analyzing transaction history for address {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll examine the activity of {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
                ResponseTemplate(
                    "Reviewing the transaction patterns for {address}.", ["address"]
                ),
                ResponseTemplate("I'll analyze the wallet activity for you."),
            ],
            Intent.ANALYZE_TRANSACTION: [
                ResponseTemplate(
                    "Analyzing transaction {tx_hash} in detail.", ["tx_hash"]
                ),
                ResponseTemplate(
                    "I'll provide an in-depth analysis of transaction {tx_hash}.",
                    ["tx_hash"],
                ),
                ResponseTemplate(
                    "Examining the execution of transaction {tx_hash} on the {blockchain} blockchain.",
                    ["tx_hash", "blockchain"],
                ),
            ...
            Intent.MONITOR_ADDRESS: [
                ResponseTemplate(
                    "Setting up monitoring for address {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll notify you of any activity related to {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
                ResponseTemplate(
                    "Starting to track transactions for {address}.", ["address"]
                ),
                ResponseTemplate("I'll set up monitoring for the address."),
            ],
            Intent.MONITOR_CONTRACT: [
                ResponseTemplate(
                    "Setting up monitoring for contract {address}.", ["address"]
                ),
                ResponseTemplate(
                    "I'll track events from the contract at {address} on the {blockchain} blockchain.",
                    ["address", "blockchain"],
                ),
                ResponseTemplate(
                    "Starting to monitor contract {address} for events.", ["address"]
                ),
            ...
            Intent.MONITOR_PRICE: [
                ...
                ResponseTemplate(
                    "I'll notify you when the price of {token} reaches your target.",
                    ["token"],
                ),
            ...
            Intent.UNKNOWN: [
                ResponseTemplate(
                    "I'm not sure what you're asking for. Could you please rephrase your question?"
                ),
                ResponseTemplate(
                    "I don't understand what you're looking for. Can you provide more details?"
                ),
                ResponseTemplate(
...
class StructuredQueryGenerator:
    ...
    async def generate_query(
        self, intent: Intent, entities: List[Entity]
    ) -> Dict[str, Any]:
        """
...
async def generate_response(
    ...
    return await response_generator.generate_response(
        intent, entities, additional_values
    )
