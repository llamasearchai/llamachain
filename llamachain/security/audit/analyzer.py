...
from llamachain.core.constants import AUDIT_SEVERITY_LEVELS, VULNERABILITY_TYPES
...
class ContractAuditor:
    ...
    def __init__(self, ruleset: Optional[RuleSet] = None):
        ...
        logger.info(
            f"Contract auditor initialized (Mythril: {self.mythril_available}, Slither: {self.slither_available})"
        )
    ...
    async def audit_contract(
        ...
        result = AuditResult(contract_address=contract_address, chain_id=chain_id)
        ...
        for rule in self.ruleset.rules:
            ...
            for match in matches:
                ...
                if match.get("function_name"):
                    ...
                    lines = source_code.splitlines()
                    for i in range(
                        max(0, line_number - 5), min(len(lines), line_number)
                    ):
