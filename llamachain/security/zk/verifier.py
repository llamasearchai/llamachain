...
if TYPE_CHECKING:
    ...
    from py_ecc.bn128 import G1, G2, add, curve_order, multiply, neg, pairing
...
class ZKVerifier:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(
                f"py_ecc library not available, some verification methods will be limited: {e}"
            )
        ...
        try:
            ...
            if self.snarkjs_available:
                ...
                logger.warning(
                    "snarkjs not available, snarkjs verification will be limited"
                )
        except (subprocess.SubprocessError, FileNotFoundError) as e:
            self.snarkjs_available = False
            logger.warning(
                f"snarkjs not available, snarkjs verification will be limited: {e}"
            )

        logger.info(
            f"ZK verifier initialized (py_ecc: {self.py_ecc_available}, snarkjs: {self.snarkjs_available})"
        )
    ...
    async def verify_proof(
        ...
        try:
            if proof_system == ProofSystem.GROTH16:
                return await self._verify_groth16(
                    proof, verification_key, public_inputs
                )
                ...
                return await self._verify_bulletproofs(
                    proof, verification_key, public_inputs
                )
            elif proof_system == ProofSystem.SNARKJS:
                return await self._verify_snarkjs(
                    proof, verification_key, public_inputs
                )
            else:
    ...
    async def _verify_groth16(
        ...
        try:
            ...
            if public_inputs:
                for i, input_value in enumerate(public_inputs):
                    if i + 1 < len(ic):
                        ...
                        vk_x = bn128.add(
                            vk_x, bn128.multiply(input_point, int(input_value))
                        )
            ...
            left_pairing = (
                bn128.pairing(a_point, b_point)
                * bn128.pairing(neg_vk_x, gamma_point)
                * bn128.pairing(neg_c, delta_point)
            )
    ...
    async def _verify_snarkjs(
        ...
        try:
            ...
            logger.error(
                f"Error verifying snarkjs proof: {process.stderr.decode('utf-8')}"
            )
            return ProofStatus.ERROR
    ...
    def generate_verification_key(
        self, proof_system: ProofSystem, circuit_params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
