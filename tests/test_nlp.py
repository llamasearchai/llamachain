...
def nlp_processor():
    ...
    with patch(
        "llamachain.nlp.processor.IntentClassifier"
    ) as mock_intent_classifier, patch(
        "llamachain.nlp.processor.EntityExtractor"
        ...
        processor._analyze_with_spacy = MagicMock(
            return_value={"entities": [], "noun_chunks": [], "tokens": []}
        )
...
class TestIntentClassifier:
    ...
    def test_get_confidence_scores(self, intent_classifier):
        ...
        scores = intent_classifier.get_confidence_scores(
            "What is the transaction 0x123456789abcdef?"
        )
...
class TestEntityExtractor:
    ...
    async def test_extract(self, entity_extractor):
        ...
        entities = await entity_extractor.extract(
            "What is the transaction 0x123456789abcdef?", Intent.GET_TRANSACTION
        )
    ...
    async def test_extract_token(self, entity_extractor):
        ...
        # Extract entities
        entities = await entity_extractor.extract(
            "What is the price of ETH?", Intent.GET_PRICE
        )
