...
class HuggingFaceModel:
    ...
    def __init__(
        ...
        logging.info(
            "Note: Using HuggingFaceModel requires installing transformers: pip install transformers"
        )
    ...
    def predict(self, text: Union[str, List[str]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise ImportError(
                "To use HuggingFaceModel, you need to install PyTorch: pip install torch"
            )
        ...
        if self.model_type == "text-classification":
                ...
                results.append(
                    {
                        ...
                        "score": score,
                        "all_scores": {
                            label_map.get(j, str(j)): float(probs[i, j])
            ...
            logging.warning(
                "Token classification output processing is simplified in this implementation"
            )
            ...
            for i in range(batch_size):
                ...
                for j, token in enumerate(tokens):
                    ...
                    token_predictions.append({"token": token, "label": label})
